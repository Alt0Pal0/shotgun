...
export function OnboardingForm({ displayName, isAdult }: { displayName: string; isAdult: boolean }) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4">
      {isAdult && (
        <Alert tone="info">
          You are linked as a supervising adult. Only create a permit profile if you are also a learner.{" "}
      ...
      <Select label="State" value="US-CA" onChange={() => undefined}>
      ...
      <Input
        ...
        hint="Found on your instruction permit. No permit number is collected."
      />
      <p className="text-xs text-muted">Timezone detected: {tz}</p>
      <Alert tone="warn" title="Before your first drive">
        {GPS_LIMITS_COPY} Enable your phone&apos;s Driving Focus; this app can only lock its own screen.
      </Alert>
