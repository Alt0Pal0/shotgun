...
export const PUT = withAuth(async ({ backend, req, params }) => {
  ...
  if (submit) await backend.rpc("track_event", { p_event: "reflection_submitted", p_props: { optional_fields_completed: Boolean(payload.went_well || payload.improve) } }).catch(() => undefined);
  return json(r);
});
