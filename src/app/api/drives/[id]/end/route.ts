...
export const POST = withAuth(async ({ backend, req, params }) => {
  ...
  const r = await backend.rpc<{ status: string }>("end_session", {
    p_session: params.id,
    p_idempotency_key: body.idempotency_key,
    p_override_reason: body.override_reason ?? null,
    p_confirmed_parked: true,
  });
  ...
  await backend
    .rpc("track_event", { p_event: "drive_ended", p_props: { override: Boolean(body.override_reason) } })
    .catch(() => undefined);
