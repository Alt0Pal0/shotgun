  ...
  return (
    <>
      <PageHeader
        ...
        subtitle={model.track ? `Permit issued ${model.track.permit_issue_date}` : "No permit profile yet"}
        action={<ExportButton learnerId={id} />}
      />
      <div className="mb-4 flex gap-2 text-sm">
        <Link className="text-accent" href={`/drives?learner=${id}`}>
          All drives
        </Link>
        <span className="text-muted">·</span>
        <Link className="text-accent" href={`/progress?learner=${id}`}>
          Progress details
        </Link>
        <span className="text-muted">·</span>
        <Link className="text-accent" href={`/records/new?learner=${id}`}>
          Add record
        </Link>
      </div>
