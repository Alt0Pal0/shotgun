...
export function LockedDrive({ initial }: { initial: LockState }) {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      try {
        ...
        /* offline: keep the lock */
      }
    };
    const t = setInterval(tick, 10_000);
    // Retry a queued end action (PRD §12: end response timed out)
    pendingEnd(lock.id).then((p) => {
      if (p) void submitEnd(p.payload as { override_reason?: string }, true);
    });
    return () => {
      on = false;
      clearInterval(t);
  ...
  async function submitEnd(payload: { override_reason?: string }, retry = false) {
    ...
    try {
      ...
      await api.post(`/api/drives/${lock.id}/end`, {
      ...
      if (code === "NOT_STATIONARY") {
        await clearPendingEnd(lock.id);
        setErr("The car doesn't look parked yet. Wait a moment, or use the override.");
        setConfirming(false);
  ...
  return (
    <div className="flex min-h-[calc(100dvh-2.5rem)] flex-col" data-testid="locked-drive">
      ...
      <div className="my-8 flex flex-col items-center" aria-live="off">
        <div
          className={`flex h-56 w-56 items-center justify-center rounded-full border-8 ${canEndNormally ? "border-accent" : "border-surface-2"}`}
        >
          <p className="numeral text-5xl font-bold" data-testid="elapsed">
            {fmtElapsed(elapsed)}
          </p>
        </div>
      ...
      <div className="card mb-4 p-4 text-center">
        <p className="text-xs font-semibold uppercase tracking-wider text-amber">Stay focused</p>
        <p className="text-sm">{SAFETY_LOCK_COPY.body}</p>
      </div>
          ...
          <strong>
            {!isRecorder
              ...
              : state.permission === "denied"
                ...
                : state.quality === "GOOD"
                  ? "good"
                  : state.quality === "LIMITED"
                    ? "limited"
                    : "no signal"}
          </strong>
        </li>
        <li className="card p-2">
          Network: <strong>{state.online ? "online" : "offline — recording locally"}</strong>
        </li>
        <li className="card p-2">
          Samples: <strong className="numeral">{state.sampleCount}</strong>
          {state.pendingUpload > 0 && <span className="text-muted"> ({state.pendingUpload} to upload)</span>}
        </li>
        <li className="card p-2">
          Screen:{" "}
          <strong>
            {state.wakeLock === "active" ? "kept awake" : state.wakeLock === "released" ? "may sleep" : "no wake lock"}
          </strong>
        </li>
      </ul>
      ...
      {state.error && (
        <div className="mb-3">
          <Alert tone="warn">{state.error}. Time is still being recorded.</Alert>
        </div>
      )}
      {err && (
        <div className="mb-3">
      ...
      <div className="mt-auto space-y-3">
        {!confirming ? (
          <>
            <p className="text-center text-xs text-muted" aria-live="polite">
              {canEndNormally ? "Parked — you can end the drive" : `End unlocks after 30 s parked (${stationaryPct}%)`}
            </p>
            <HoldButton
              label={canEndNormally ? "Hold to end drive" : "We're parked (keep still)"}
              disabled={!canEndNormally || busy}
          ...
          <div className="card space-y-3 p-4">
            ...
            <Button size="lg" block loading={busy} onClick={() => submitEnd({})} data-testid="confirm-end">
              Yes, we&apos;re parked — end drive
            </Button>
        ...
        {!overrideOpen ? (
          ...
          >
            GPS or phone problem? End with an override
          </button>
        ) : (
          <div className="card space-y-2 p-3">
            <label className="block text-xs font-medium" htmlFor="override">
              Why are you ending without parked detection? (recorded for the adult review)
            </label>
            <input
              id="override"
              ...
              placeholder="e.g., GPS lost in parking garage"
            />
            <Button
              variant="secondary"
              block
              ...
              onClick={() => {
                setConfirming(false);
                void submitEnd({ override_reason: reason.trim() });
              }}
            >
              End with override
            </Button>
        ...
        <a
          href="tel:911"
          className="tap block w-full rounded-xl border border-rose/60 py-3 text-center text-sm font-semibold text-rose"
        >
          Emergency: call 911
        </a>
      </div>
