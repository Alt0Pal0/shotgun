...
export default async function LivePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [view, skills] = await Promise.all([backend.rpc<LiveView | null>("live_view", { p_session: id }), backend.rpc<Skill[]>("skills_list")]);
  if (!view) {
    ...
    return <><PageHeader title="Live view unavailable" /><Alert tone="warn">You are linked to this learner but not authorized to view this drive live. The learner can allow remote live view in their settings.</Alert></>;
  }
