...
export default async function ReflectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!["AWAITING_LEARNER_REFLECTION", "RETURNED_FOR_REVISION", "RECOVERY_REQUIRED"].includes(s.status))
    redirect(`/drives/${id}`);
  return (
    <>
      <PageHeader
        eyebrow="Self-review"
        title="How did it go?"
        subtitle="Your reflection stays with this drive. Your adult sees it when you send it for review."
      />
      {s.status === "RETURNED_FOR_REVISION" && s.review?.next_focus && (
        <div className="mb-4">
          <Alert tone="warn" title="Returned by your adult">
            {s.review.next_focus}
          </Alert>
        </div>
      )}
