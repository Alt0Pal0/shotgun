...
async function runAs<T>(
  ...
  fn: (q: (sql: string, args?: unknown[]) => Promise<{ rows: Record<string, unknown>[] }>) => Promise<T>,
): Promise<T> {
  guardProduction();
...
export const localBackend: Backend = {
  ...
  async serviceRpc<T>(fn: string, args: Record<string, unknown> = {}) {
    return callRpc<T>(null, "service_role", fn, args);
  },
  async serviceSamples(sessionId: string) {
    return runAs(
      ...
      async (q) =>
        (
          await q(
            "select sequence_no, recorded_at, latitude, longitude, accuracy_m, speed_mps, heading_deg from location_samples where session_id = $1 order by sequence_no",
            [sessionId],
          )
  ...
  async signUp({ email, password, displayName, role }): Promise<AuthResult> {
    ...
    try {
      ...
      return {
        ok: false,
        error: err.code === "23505" ? "An account with this email already exists" : "Could not create account",
      };
    }
  },
  async signIn({ email, password }): Promise<AuthResult> {
    const { rows } = await pool.query(
      "select id from auth.users where email = $1 and raw_user_meta_data ->> 'password_hash' = crypt($2, raw_user_meta_data ->> 'password_hash')",
  ...
  async resendVerification(email): Promise<AuthResult> {
    const { rows } = await pool.query(
      "select raw_user_meta_data ->> 'verify_token' as t from auth.users where email = $1",
      [email.toLowerCase()],
    );
    return {
      ok: true,
      needsVerification: true,
      devVerifyUrl: rows[0]?.t ? `/auth/local-verify?token=${rows[0].t}` : undefined,
    };
  },
  ...
  async updatePassword(newPassword): Promise<AuthResult> {
    ...
    await pool.query(
      "update auth.users set raw_user_meta_data = raw_user_meta_data || jsonb_build_object('password_hash', crypt($2, gen_salt('bf'))) where id = $1",
      [uid, newPassword],
    );
    return { ok: true };
  },
...
export async function localVerifyEmail(token: string): Promise<boolean> {
  const { rows } = await pool.query(
    "update auth.users set email_confirmed_at = now() where raw_user_meta_data ->> 'verify_token' = $1 returning id",
    [token],
  );
