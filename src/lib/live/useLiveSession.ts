...
export function useLiveSession(sessionId: string) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      setView(v);
      setError(null);
      setConn((c) => ({ ...c, connected: true, lastUpdateAt: Date.now() }));
      if (v.live.latest_latitude != null && v.live.latest_longitude != null) {
        ...
        setTrail((t) => {
          const last = t.at(-1);
          return last && last[0] === pt[0] && last[1] === pt[1] ? t : [...t.slice(-400), pt];
        });
      }
    } catch (e) {
  ...
  useEffect(() => {
    ...
    if (sb) {
      channel = sb
        ...
        .on(
          "postgres_changes",
          { event: "*", schema: "public", table: "live_session_state", filter: `session_id=eq.${sessionId}` },
          () => void refresh(),
        )
        .on(
          "postgres_changes",
          { event: "INSERT", schema: "public", table: "drive_observations", filter: `session_id=eq.${sessionId}` },
          () => void refresh(),
        )
        .on(
          "postgres_changes",
          { event: "UPDATE", schema: "public", table: "drive_sessions", filter: `id=eq.${sessionId}` },
          () => void refresh(),
        )
        .subscribe((status: string) =>
          setConn((c) => ({
            ...c,
            mode: status === "SUBSCRIBED" ? "realtime" : "polling",
            connected: status === "SUBSCRIBED" ? true : c.connected,
          })),
        );
    }
    return () => {
