...
export interface NightRule {
  after_sunset_minutes: number;
  before_sunrise_minutes: number;
}
...
export function computeNightMinutes(
  ...
  startedAt: Date,
  endedAt: Date,
  rule: NightRule = DEFAULT_NIGHT_RULE,
  maxGapMs = 5 * 60_000,
): NightResult {
  ...
  if (pts.length === 0) {
    return {
      night_minutes: 0,
      gap_minutes: Math.round((endedAt.getTime() - startedAt.getTime()) / 60_000),
      segments: [],
    };
  }
  let darkMs = 0,
  ...
  const anchors = [
    { t: startedAt.getTime(), lat: pts[0].latitude, lng: pts[0].longitude },
    ...pts.map((p) => ({ t: Date.parse(p.recorded_at), lat: p.latitude, lng: p.longitude })),
    { t: endedAt.getTime(), lat: pts[pts.length - 1].latitude, lng: pts[pts.length - 1].longitude },
  ];
  for (let i = 1; i < anchors.length; i++) {
    ...
    for (let k = 0; k < steps; k++) {
      const t0 = a.t + (span * k) / steps,
        t1 = a.t + (span * (k + 1)) / steps;
