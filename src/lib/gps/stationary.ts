...
export interface StationaryState { since: number | null; anchorLat: number | null; anchorLng: number | null }

export function initialStationaryState(): StationaryState { return { since: null, anchorLat: null, anchorLng: null }; }

/** Client-side mirror of the server's stationary detector used to enable the End control optimistically. */
export function updateStationary(state: StationaryState, lat: number, lng: number, speedMps: number | null, t: number): StationaryState {
