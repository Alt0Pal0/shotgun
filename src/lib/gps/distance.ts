...
export interface FilterResult {
  ...
  flagged: {
    from: LocationSample;
    to: LocationSample;
    reason: "IMPLAUSIBLE_SPEED" | "TIMESTAMP_ORDER";
...
export function filterSamples(samples: LocationSample[]): FilterResult {
  ...
  for (const s of sorted) {
    if (seen.has(s.sequence_no)) {
      rejected.push({ sample: s, reason: "DUPLICATE_SEQUENCE" });
      continue;
    }
    ...
    if (
      ...
      Number.isNaN(Date.parse(s.recorded_at))
    ) {
      rejected.push({ sample: s, reason: "INVALID" });
      continue;
    }
    if (s.accuracy_m != null && s.accuracy_m > MAX_ACCEPTED_ACCURACY_M) {
      rejected.push({ sample: s, reason: "INACCURATE" });
      continue;
    }
    accepted.push(s);
  }
  for (let i = 1; i < accepted.length; i++) {
    ...
    if (dt <= 0) {
      flagged.push({ from: a, to: b, reason: "TIMESTAMP_ORDER" });
      continue;
    }
    const d = haversineMeters(a.latitude, a.longitude, b.latitude, b.longitude);
    const v = d / dt;
    if (v > MAX_PLAUSIBLE_SPEED_MPS)
      flagged.push({ from: a, to: b, reason: "IMPLAUSIBLE_SPEED", implied_speed_mps: v });
  }
...
export function classifyQuality(samples: LocationSample[], windowMs = 60_000, now = Date.now()): GpsQuality {
  const recent = samples.filter(
    (s) =>
      now - Date.parse(s.recorded_at) <= windowMs && (s.accuracy_m == null || s.accuracy_m <= MAX_ACCEPTED_ACCURACY_M),
  );
...
export function classifyDriveQuality(
  accepted: LocationSample[],
  startedAt: Date,
  endedAt: Date,
): { quality: GpsQuality; coverage: number; longest_gap_s: number } {
  ...
  const quality: GpsQuality =
    accepted.length < MIN_USABLE_POINTS
      ? "NONE"
      : coverage >= 0.9 && medianAcc <= 30 && longest <= 120
        ? "GOOD"
        : "LIMITED";
