...
export async function syncSamples(sessionId: string, deviceId: string, batchSize = 100): Promise<SyncResult> {
  let uploaded = 0,
    ...
    can_end: boolean | undefined,
    stationary_seconds: number | undefined;
  for (;;) {
    ...
    try {
      const r = await api.post<{
        ...
        stationary_seconds: number;
        ignored?: boolean;
      }>(`/api/drives/${sessionId}/samples`, { device_id: deviceId, samples: batch });
      await markAcked(
        sessionId,
        batch.map((s) => s.sequence_no),
      );
      uploaded += r.accepted;
      duplicates += r.duplicates;
      status = r.status;
      can_end = r.can_end;
      stationary_seconds = r.stationary_seconds;
      if (r.ignored) break;
    } catch (e) {
