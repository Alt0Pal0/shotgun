...
interface RecorderDB extends DBSchema {
  samples: { key: [string, number]; value: LocationSample & { session_id: string; acked_at?: number }; indexes: { by_session: string } };
  pending_actions: { key: string; value: { id: string; session_id: string; type: "end"; payload: Record<string, unknown>; created_at: number } };
...
export async function unackedSamples(sessionId: string, limit = 200): Promise<LocationSample[]> {
  ...
  return all.filter((s) => s.acked_at == null).sort((a, b) => a.sequence_no - b.sequence_no).slice(0, limit)
    .map((s) => ({ sequence_no: s.sequence_no, recorded_at: s.recorded_at, latitude: s.latitude, longitude: s.longitude, accuracy_m: s.accuracy_m, speed_mps: s.speed_mps, heading_deg: s.heading_deg }));
}
...
export async function cleanupAcked(olderThanMs = 24 * 3600_000): Promise<void> {
  ...
  while (cur) { if (cur.value.acked_at != null && cur.value.acked_at < cutoff) await cur.delete(); cur = await cur.continue(); }
  await tx.done;
}

export async function queueEnd(sessionId: string, payload: Record<string, unknown>): Promise<void> {
  const d = await db();
  await d.put("pending_actions", { id: `end:${sessionId}`, session_id: sessionId, type: "end", payload, created_at: Date.now() });
}
export async function pendingEnd(sessionId: string) { return (await db()).get("pending_actions", `end:${sessionId}`); }
export async function clearPendingEnd(sessionId: string) { await (await db()).delete("pending_actions", `end:${sessionId}`); }
