import { rulesetConfigSchema, type Contribution, type Evaluation, type EvaluationInput, type Requirement, type RequirementCard, type RulesetConfig } from "./types";
...
function sum(contribs: Contribution[], key: string, category?: string): number {
  return contribs
    .filter((c) => c.requirement_key === key && (c.evidence_state ?? "FINAL") === "FINAL" && (category ? c.category === category : true))
...
export function evaluateRequirement(req: Requirement, input: EvaluationInput): RequirementCard {
  ...
  switch (req.type) {
    case "duration_total": {
      ...
      return { key: req.key, label: req.label, type: req.type, unit: "minutes", target: req.target_minutes, approved, remaining,
        percent: Math.min(100, Math.round((approved / req.target_minutes) * 100)), complete: remaining === 0, blocking: true };
    }
    ...
    case "waiting_period": {
      ...
      if (!startRaw) {
        return { key: req.key, label: req.label, type: req.type, unit: "days", target: null, approved: 0, remaining: null, percent: null,
          complete: false, eligible_on: null, blocking: true, note: `Missing ${req.start_field}` };
      }
      const start = new Date(`${startRaw.slice(0, 10)}T00:00:00Z`);
      const end = req.target_months ? addMonths(start, req.target_months) : new Date(start.getTime() + (req.target_days ?? 0) * 86_400_000);
      ...
      return { key: req.key, label: req.label, type: req.type, unit: "days", target: totalDays, approved: elapsedDays, remaining,
        percent: totalDays === 0 ? 100 : Math.min(100, Math.round((elapsedDays / totalDays) * 100)), complete: now.getTime() >= end.getTime(),
        eligible_on: isoDate(end), blocking: true, note: req.until_then_restriction };
    }
    case "event_count": {
      const approved = input.events?.[req.event_key] ?? 0;
      const remaining = Math.max(0, req.target_count - approved);
      return { key: req.key, label: req.label, type: req.type, unit: "count", target: req.target_count, approved, remaining,
        percent: Math.min(100, Math.round((approved / req.target_count) * 100)), complete: remaining === 0, blocking: true };
    }
    case "document_or_attestation": {
      const has = (input.evidence ?? []).includes(req.evidence_key);
      return { key: req.key, label: req.label, type: req.type, unit: "boolean", target: 1, approved: has ? 1 : 0, remaining: has ? 0 : 1,
        percent: has ? 100 : 0, complete: has, blocking: true };
    }
    case "restriction":
      return { key: req.key, label: req.label, type: req.type, unit: "none", target: null, approved: 0, remaining: null, percent: null,
        complete: true, blocking: false, note: req.rule };
    case "recommendation": {
      const approved = sum(contributions, req.key);
      const target = req.target_minutes ?? null;
      return { key: req.key, label: req.label, type: req.type, unit: target ? "minutes" : "none", target, approved,
        remaining: target ? Math.max(0, target - approved) : null, percent: target ? Math.min(100, Math.round((approved / target) * 100)) : null,
        complete: target ? approved >= target : true, blocking: false, note: req.note };
    }
  }
}
