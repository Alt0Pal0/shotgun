...
export type Handler = (ctx: { req: Request; backend: Backend; user: SessionUser; params: Record<string, string> }) => Promise<Response>;
...
export function withAuth(handler: Handler, opts: { allowUnverified?: boolean } = {}) {
  return async (req: Request, ctx: { params: Promise<Record<string, string>> }): Promise<Response> => {
    try {
      ...
      if (!user.emailVerified && !opts.allowUnverified) throw new AppError("UNVERIFIED", "Verify your email to continue", 403);
...
export function errorResponse(e: unknown): Response {
  if (e instanceof AppError) return NextResponse.json({ error: { code: e.code, message: e.message, hint: e.hint } }, { status: e.status });
  const err = e as { name?: string; issues?: unknown; message?: string };
  if (err.name === "ZodError") return NextResponse.json({ error: { code: "VALIDATION", message: "Invalid input", issues: err.issues } }, { status: 422 });
...
export async function parseBody<T>(req: Request, schema: ZodType<T>): Promise<T> {
  const raw: unknown = await req.json().catch(() => { throw new AppError("VALIDATION", "Body must be JSON", 400); });
