...
const c = (requirement_key: string, amount: number, extra: Partial<Contribution> = {}): Contribution => ({
  requirement_key,
  amount,
  unit: "minutes",
  ...extra,
...
describe("California ruleset (production)", () => {
  ...
  it("computes 50/10/6 and permit hold from approved contributions only", () => {
    const ev = evaluate({
      config,
      now,
      fields: { permit_issue_date: "2026-03-01" },
      contributions: [
        c("supervised_total", 1020),
        c("night_subset", 120),
        c("professional_training", 120),
        c("supervised_total", 60, { evidence_state: "VOIDED" }),
      ],
    });
    const byKey = Object.fromEntries(ev.cards.map((x) => [x.key, x]));
    expect(byKey.supervised_total).toMatchObject({
      approved: 1020,
      remaining: 1980,
      target: 3000,
      percent: 34,
  ...
  it("night minutes are capped by supervised minutes and professional time never feeds the 50-hour total", () => {
    const ev = evaluate({
      config,
      now,
      fields: { permit_issue_date: "2025-01-01" },
      contributions: [c("night_subset", 700), c("supervised_total", 500), c("professional_training", 3000)],
    });
    const byKey = Object.fromEntries(ev.cards.map((x) => [x.key, x]));
...
describe("structural fixtures for other states (never selectable in production)", () => {
  it("Texas: separate classroom, instructor-driving, observation and supervised-practice categories", () => {
    ...
    const ev = evaluate({
      config,
      now,
      fields: { permit_issue_date: "2026-01-01" },
      contributions: [
        c("classroom", 1920, { category: "classroom" }),
        c("instructor_driving", 60, { category: "driving" }),
        c("observation", 60, { category: "observation" }),
  ...
  it("Florida: long permit hold plus phase-dependent time-of-day restriction and a document requirement", () => {
    ...
    const ev = evaluate({
      config,
      now,
      fields: { permit_issue_date: "2026-06-01" },
      contributions: [],
      evidence: ["tlsae_certificate"],
    });
    const byKey = Object.fromEntries(ev.cards.map((x) => [x.key, x]));
    expect(byKey.permit_hold.eligible_on).toBe("2027-06-01");
    expect(byKey.daylight_phase).toMatchObject({
      eligible_on: "2026-09-01",
      complete: false,
      note: "Driving only during daylight hours",
    });
  ...
  it("New York: professional hours can count toward the total and regional restriction is represented", () => {
    ...
    const ev = evaluate({
      ...
      now,
      fields: { permit_issue_date: "2026-01-01" },
      contributions: [c("supervised_total", 600), c("professional_within_total", 600)],
      evidence: [],
    });
    const byKey = Object.fromEntries(ev.cards.map((x) => [x.key, x]));
    expect(config.requirements.find((r) => r.key === "supervised_total")).toMatchObject({
      eligible_session_types: ["FAMILY_SUPERVISED", "PROFESSIONAL_INSTRUCTION"],
    });
    expect(byKey.professional_within_total.complete).toBe(true);
    expect(byKey.nyc_restriction).toMatchObject({
      blocking: false,
      note: "No permit driving in New York City boroughs",
    });
  ...
  it("Pennsylvania: a poor-weather subset in addition to night", () => {
    ...
    const ev = evaluate({
      ...
      now,
      fields: { permit_issue_date: "2026-01-01" },
      contributions: [c("supervised_total", 1000), c("night_subset", 100), c("poor_weather_subset", 50)],
    });
    const byKey = Object.fromEntries(ev.cards.map((x) => [x.key, x]));
