...
const s = (seq: number, t: number, lat: number, lng: number, acc = 8, spd = 10): LocationSample => ({
  sequence_no: seq,
...
describe("GPS filtering and distance", () => {
  ...
  it("rejects inaccurate points, drops duplicate sequences, flags implausible speed and timestamp order", () => {
    const samples = [
      s(0, T0, 37.77, -122.41),
      s(1, T0 + 5000, 37.7705, -122.41, 150),
      s(2, T0 + 10000, 37.771, -122.41),
      s(2, T0 + 10000, 37.771, -122.41),
      s(3, T0 + 15000, 37.85, -122.41),
      s(4, T0 + 14000, 37.851, -122.41),
    ];
  ...
  it("returns null distance with fewer than five usable points but retains duration elsewhere", () => {
    const r = filterSamples([
      s(0, T0, 37.77, -122.41),
      s(1, T0 + 5000, 37.771, -122.41),
      s(2, T0 + 10000, 37.772, -122.41),
    ]);
...
describe("stationary detection", () => {
  it("requires ~30 s of slow, low-displacement samples; a brief stop does not count; moving resets", () => {
    ...
    t += 5000;
    st = updateStationary(st, 37.7705, -122.41, 12, t); // moved 55 m
    expect(stationarySeconds(st, t)).toBe(0);
    for (let i = 1; i <= 7; i++) {
      t += 5000;
      st = updateStationary(st, 37.7705, -122.41, 0.3, t);
    }
...
describe("night-minute segmentation", () => {
  ...
  it("does not auto-classify long GPS gaps; surfaces them for the adult", () => {
    ...
    const samples = [
      s(0, start, sf.lat, sf.lng),
      s(1, start + 2 * 60_000, sf.lat, sf.lng),
      s(2, start + 30 * 60_000, sf.lat, sf.lng),
      s(3, start + 32 * 60_000, sf.lat, sf.lng),
    ];
