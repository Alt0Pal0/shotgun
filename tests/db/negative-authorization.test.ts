...
import { as, asService, closePool, createUser, expectDenied, linkAdult, makeLearner, registerDevice, rpc, rpcService, sample, select, startActiveSession } from "./harness";
...
describe("negative authorization", () => {
  ...
  it("an unrelated adult cannot access a learner, subscribe to the live session, or review", async () => {
    ...
    expect(await select(stranger, "select * from live_session_state where session_id = $1", [session.id])).toHaveLength(0);
    expect(await select(stranger, "select * from drive_observations where session_id = $1", [session.id])).toHaveLength(0);
    expect(await select(stranger, "select * from session_participants where session_id = $1", [session.id])).toHaveLength(0);
    await expectDenied(rpc(stranger, "add_observation", [session.id, JSON.stringify({ observation_type: "DID_WELL" })]), "NOT_FOUND");
    await expectDenied(rpc(stranger, "accept_session", [session.id, JSON.stringify({ designated_supervisor: true, physically_present: true, vehicle_parked: true, ready: true })]), "NOT_FOUND");
    await expectDenied(rpc(stranger, "end_session", [session.id, randomUUID(), null, true]), "FORBIDDEN");
    await expectDenied(rpc(stranger, "review_session", [session.id, JSON.stringify({ decision: "VOIDED", correction_reason: "x" }), randomUUID()]), "NOT_FOUND");
  ...
  it("a revoked adult loses live and historical access", async () => {
    ...
    await expectDenied(rpc(adult, "add_observation", [session.id, JSON.stringify({ observation_type: "DID_WELL" })]), "NOT_FOUND");
    ...
    await expectDenied(rpc(learner, "request_session", [JSON.stringify({ supervisor_id: adult.id, recorder_device_id: dev, idempotency_key: randomUUID() })]), "FORBIDDEN");
  });
  ...
  it("a learner cannot approve their own drive or write reviews/contributions directly", async () => {
    ...
    await rpcService("record_route_processing", [session.id, JSON.stringify({ gps_quality: "NONE", processing_version: "route-v1", night_algorithm_version: "night-v1" })]);
    await rpc(learner, "save_reflection", [session.id, JSON.stringify({ rating: 5 }), true]);
    await expectDenied(rpc(learner, "review_session", [session.id, JSON.stringify({ decision: "APPROVED", rating: 5 }), randomUUID()]));
    await expectDenied(as(learner, (c) => c.query("insert into supervisor_reviews (session_id, reviewer_id, decision) values ($1, $2, 'APPROVED')", [session.id, learner.id])));
    await expectDenied(as(learner, (c) => c.query("insert into requirement_contributions (session_id, learner_id, requirement_key, amount, ruleset_version, evidence_type, approved_by) values ($1, $2, 'supervised_total', 3000, 'x', 'GPS', $2)", [session.id, learner.id])));
    await expectDenied(as(learner, (c) => c.query("update drive_sessions set status = 'APPROVED', credited_duration_minutes = 3000 where id = $1", [session.id])));
    expect(await select(learner, "select * from requirement_contributions where learner_id = $1", [learner.id])).toHaveLength(0);
  });
  ...
  it("a learner cannot retrieve adult observations or live state during an active drive, even as the author-adjacent party", async () => {
    ...
    const { adult } = await linkAdult(learner);
    const { session } = await startActiveSession(learner, adult);
    await rpc(adult, "add_observation", [session.id, JSON.stringify({ observation_type: "NEEDS_PRACTICE", note: "mirror" })]);
    expect(await select(learner, "select * from drive_observations where session_id = $1", [session.id])).toHaveLength(0);
    expect(await select(learner, "select * from live_session_state where session_id = $1", [session.id])).toHaveLength(0);
    ...
    const s = await select<{ status: string }>(learner, "select status from drive_sessions where id = $1", [session.id]);
  ...
  it("a remote viewer cannot create verified observations; only the in-car supervisor can", async () => {
    ...
    const parts = await select<{ user_id: string; role: string }>(learner, "select user_id, role from session_participants where session_id = $1", [session.id]);
    expect(parts.find((p) => p.user_id === remote.id)?.role).toBe("REMOTE_VIEWER");
    expect(await select(remote, "select * from live_session_state where session_id = $1", [session.id])).toHaveLength(1);
    const note = await rpc<{ verification_level: string }>(remote, "add_observation", [session.id, JSON.stringify({ observation_type: "NOTE", note: "looked smooth from the cam" })]);
    expect(note.verification_level).toBe("UNVERIFIED");
    await expectDenied(rpc(remote, "add_observation", [session.id, JSON.stringify({ observation_type: "DID_WELL", verification_level: "VERIFIED" })]), "FORBIDDEN");
    const verified = await rpc<{ verification_level: string }>(inCar, "add_observation", [session.id, JSON.stringify({ observation_type: "DID_WELL" })]);
  ...
  it("a user cannot upload samples to another learner's session or from a non-recorder device", async () => {
    ...
    await expectDenied(rpc(adult, "ingest_samples", [session.id, adultDevice, JSON.stringify([sample(0, new Date(), 37.77, -122.41)])]), "FORBIDDEN");
    await expectDenied(rpc(adult, "ingest_samples", [session.id, device, JSON.stringify([sample(0, new Date(), 37.77, -122.41)])]), "FORBIDDEN");
    const other = await makeLearner();
    const otherDevice = await registerDevice(other);
    await expectDenied(rpc(other, "ingest_samples", [session.id, otherDevice, JSON.stringify([sample(0, new Date(), 37.77, -122.41)])]), "FORBIDDEN");
    await expectDenied(as(other, (c) => c.query("insert into location_samples (session_id, device_id, sequence_no, recorded_at, latitude, longitude) values ($1, $2, 0, now(), 1, 1)", [session.id, otherDevice])));
  });
  ...
  it("precise route and live location are never readable anonymously", async () => {
    ...
    for (const table of ["location_samples", "live_session_state", "drive_routes", "drive_sessions", "drive_observations", "profiles"]) {
  ...
  it("learner cannot bypass the lock by mutating session state directly", async () => {
    ...
    const { adult } = await linkAdult(learner);
    const { session } = await startActiveSession(learner, adult);
    await expectDenied(as(learner, (c) => c.query("update drive_sessions set status = 'ENDED' where id = $1", [session.id])));
    await expectDenied(as(learner, (c) => c.query("delete from drive_sessions where id = $1", [session.id])));
    await expectDenied(as(learner, (c) => c.query("update session_participants set can_view_live = true where session_id = $1", [session.id])));
    await expectDenied(as(learner, (c) => c.query("insert into session_participants (session_id, user_id, role, can_view_live) values ($1, $2, 'IN_CAR_SUPERVISOR', true)", [session.id, learner.id])));
    const s = await select<{ status: string }>(learner, "select status from drive_sessions where id = $1", [session.id]);
    expect(s[0].status).toBe("ACTIVE");
  });
  ...
  it("invitations are single-use, expiring, revocable and cannot be self-accepted", async () => {
    ...
    await asService((c) => c.query("update relationship_invitations set expires_at = now() - interval '1 day' where id = $1", [inv3.id]));
    await expectDenied(rpc(a2, "accept_invitation", [inv3.token, attest]), "INVALID_STATE");
    // token hash is never the raw token
    const rows = await select<{ token_hash: string }>(learner, "select token_hash from relationship_invitations where id = $1", [inv.id]);
  ...
  it("only the designated supervisor can accept a requested drive", async () => {
    ...
    const req = await rpc<{ id: string }>(learner, "request_session", [JSON.stringify({ supervisor_id: designated.id, recorder_device_id: device, idempotency_key: randomUUID() })]);
    const conf = JSON.stringify({ designated_supervisor: true, physically_present: true, vehicle_parked: true, ready: true });
    await expectDenied(rpc(otherLinked, "accept_session", [req.id, conf]), "FORBIDDEN");
    await expectDenied(rpc(designated, "accept_session", [req.id, JSON.stringify({ designated_supervisor: true, physically_present: false, vehicle_parked: true, ready: true })]), "VALIDATION");
